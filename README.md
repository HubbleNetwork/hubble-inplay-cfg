...
Given three inputs — `key0`, `rot_exp`, and advertising `interval` — this tool emits a
...
```
hubble-inplay-cfg [--key <hex-or-b64>] [--rot-exp <int>] [--interval <seconds>] [--payload <hex>] [-o <path>]
```
...
```bash
hubble-inplay-cfg --key "$KEY" --rot-exp 12 --interval 1 > device.cfg
```

Use a multi-byte payload (e.g. a 3-byte tag identifier):

```bash
hubble-inplay-cfg --key "$KEY" --rot-exp 10 --interval 2 --payload FF01AB
```
